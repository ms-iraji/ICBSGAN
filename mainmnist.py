...
def train(trainloader,unlabelledloader, model, ema_model, optimizer, epoch, filep):
    ...
    for o,(u, _) in enumerate(unlabelledloader):
        ...
        #+lentropyu+ldsu     
        #lentropyf+ldsf+
        loss = C_unlabeled_loss+0.1*C_fake_lossmix+0.1* C_fake_loss+class_loss+0.1*mixup_consistency_loss#+0.1*C_fake_lossmix+C_labeled_loss +C_unlabeled_loss+generated_weight(epoch)*C_u_lossmix
        meters.update('loss', loss.item())
    ...
    with torch.no_grad():             
                vutils.save_image(d3,
                'experiments/s/c/inter_samples%s.png' % (epoch + 1),
                normalize=True)
                vutils.save_image(d3,
                'experiments/s/c/inter_samples%s.pdf' % (epoch + 1),
                normalize=True)       

                vutils.save_image(d2,
                'experiments/s/dg/inter_samples%s.png' % (epoch + 1),
                normalize=True)
                vutils.save_image(d2,
                'experiments/s/dg/inter_samples%s.pdf' % (epoch + 1),
                normalize=True)     
